...
Client.prototype.mkdir = function(path, cb) {
...
Client.prototype.readdir = function(path, cb) {
...
Client.prototype.rmdir = function deletePath(path, cb) {
...
Client.prototype.set = function(path, value, options, cb) {
  if (typeof options == 'function') {
    cb = options;
    options = undefined;
  }
  
  this._set(path, value, options, function (err, result) {
    if (err) {
      return cb(err);
    }
...
Client.prototype.get = function getValue(path, cb) {
  this._get(path, function (err, result) {
    if (err) {
      return cb(err);
    }
...
Client.prototype.delete = function(path, cb) {
...
Client.prototype._get = function(path, options, cb) {
  ...
  //url = 'http://localhost:4001/v2/version';
  debug('GET %s', url);
  request({ url: url, pool: { maxSockets: 9999 } }, function(err, resp, body) {
    if (err) { return cb(err); }
    
    try {
      body = JSON.parse(body);
    } catch (_) {
      return cb(new Error('Invalid content received from server'));
    }
...
Client.prototype._set = function(path, data, options, cb) {
  ...
  request.put({ url: url, pool: { maxSockets: 9999 } }, function (err, resp, body) {
    if (err) {
      return cb(err);
    }
...
Client.prototype._delete = function(path, options, cb) {
  ...
  var url = this._url + path;
  debug('DELETE %s', url);
  request.del({ url: url, pool: { maxSockets: 9999 } }, function (err, resp, body) {
    if (err) {
      return cb(err);
    }
